...
echo
echo "# Building httpstat binary"
go build -o httpstat . || { echo "build failed"; exit 1; }
...
HTTPSTAT_DEBUG=true main $http_url | grep -q 'HTTPSTAT_DEBUG'
assert_exit 0
...
title "HTTPSTAT_SHOW_BODY=true, POST --data-urlencode \"a=中文\""
...
echo "$out" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['schema_version']==1"
assert_exit 0
...
assert_exit 0
python3 -c "import json; d=json.load(open('$tmpout')); assert d['schema_version']==1"
assert_exit 0
rm -f "$tmpout"

title "NO_COLOR disables ANSI escapes"
out=$(NO_COLOR=1 main $http_url)
if echo "$out" | python3 -c "import sys; sys.exit(0 if '\x1b[' in sys.stdin.read() else 1)" 2>/dev/null; then
